...
public class PlayerScript : MonoBehaviour
{
	...
	public List<Transform> outsideTriggers;
	...
	private void MouseMove()
    {
        if (entered != null)
		{
            this.playerRotation = Quaternion.Euler(playerRotation.eulerAngles.x, entered.rotation.eulerAngles.y+180f, playerRotation.eulerAngles.z);
			entered = null;
        }
		Debug.Log(cameraScript.lookBehind);
	...
	private void PlayerMove()
	{
		...
		this.cc.Move(this.moveDirection + movementOffset);
    ...
    private void StaminaCheck()
	{
		if (stamina <= 0f)
		{
			fill.color = Color.Lerp(fill.color, Color.red, (0.3f * Time.deltaTime) * 25f);
            auxText.color = Color.Lerp(fill.color, Color.yellow, (0.3f * Time.deltaTime) * 25f);
        }
		else
        {
            fill.color = Color.Lerp(fill.color, Color.yellow, (0.3f * Time.deltaTime) * 25f);
            auxText.color = Color.Lerp(fill.color, Color.yellow, (0.3f * Time.deltaTime) * 25f);
        }
        ...
        if (this.cc.velocity.magnitude > 0.1f)
		{
			if (Singleton<InputManager>.Instance.GetActionKey(InputAction.Run))
			{
				if (stamina > 0f)
                {
                    ...
                    staminaText.color = Color.Lerp(staminaText.color, Color.yellow, (0.3f * Time.deltaTime) * 25f);
                }
				if (stamina <= 0f)
				{
                    stamina = -5f;
                    staminaText.color = Color.Lerp(staminaText.color, Color.red, (0.3f * Time.deltaTime) * 25f);
                }
            }
			else
            {
                if (stamina <= 0f)
                {
                    ...
                    staminaText.color = Color.Lerp(staminaText.color, new Color(staminaText.color.r, staminaText.color.g, staminaText.color.b, 0), (0.3f * Time.deltaTime) * 25f);
            }
		}
		...
		if (outsideTriggers.Count > 0)
            this.stamina += this.staminaRate * Time.deltaTime;

        this.staminaBar.value = this.stamina / this.maxStamina * 10f;
	}
	...
	{
		if (other.CompareTag("BluePortal") && gc.currentOrangePortal != null)
		{
			cc.enabled = false;
			cameraScript.enabled = false;
            transform.position = gc.currentOrangePortal.transform.position;
			transform.position -= gc.currentOrangePortal.transform.forward * 5f;
			entered = gc.currentOrangePortal.transform;
            cc.enabled = true;
            cameraScript.enabled = true;
        }
        if (other.CompareTag("OrangePortal") && gc.currentBluePortal != null)
        {
            cc.enabled = false;
            cameraScript.enabled = false;
            transform.position = gc.currentBluePortal.transform.position;
            transform.position -= gc.currentBluePortal.transform.forward * 5f;
            entered = gc.currentBluePortal.transform;
            cc.enabled = true;
            cameraScript.enabled = true;
        }
		...
		if (other.name == "OutsideTrigger")
		{
			outsideTriggers.Add(other.transform);
		}
	}
	...
	private void OnTriggerExit(Collider other)
	{
		...
		if (other.name == "OutsideTrigger")
			outsideTriggers.Remove(other.transform);
	}
	...
	{
		gc.boots.gameObject.SetActive(true);
	...
	private IEnumerator BootTimer()
	{
        ...
        gc.boots.gameObject.SetActive(false);
        yield break;
	}
